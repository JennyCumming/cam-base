...
/**
 *  Not currently called but could be used to make changes on every login, including new registrations and existing users now logging in by Raven
 *
 */
 ...
 */
function raven_raven_register_alter(&$edit, $account) {
 ...
 * @see hook_raven_login_alter()
 */
function raven_raven_migrate_alter(&$edit, $account) {
  // Change the email address
  //$edit['mail'] = 'example@example.com';
}
 ...
 * @see hook_raven_migrate_alter()
 */
function raven_raven_login_alter(&$edit, &$account) {
  // Change the email address
  //$edit['mail'] = 'example@example.com';
